...
def global_to_element_21x21(element_matrix,global_matrix):
    ...
    for i in np.arange(1,1762,4):
        if (i+3)%84==0 and i!=1761: ##if the node is on the boundary element
            ##skip two to get to the next global node (boundaries done sep.)
            ...
            j+=2
        elif i == 1761:##since the last element would only have 3 nodes
            ##and not the next (the 4th)
            element_matrix[i] = global_matrix[j] 
            element_matrix[i+1] = global_matrix[j+22]
            element_matrix[i+2] = global_matrix[j+21]
        else:
...
def element_to_global_21x21(R_phi, p_phi_R_phi):
    ...
    ##assembling p_phi_R_phi's
    ##global_phi (the derivative variable)= column, R = row
    ##summing corners
